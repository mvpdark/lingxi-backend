...
class AgentOrchestrator:
    ...
    async def run_image_generator(
        ...
        try:
            ...
            # 如果是 base64 格式，需落地为可访问的 URL
            # （避免超大 data URI 通过 WebSocket 传输失败）。
            # 优先走 WebDAV 租户存储（多租户隔离 + 签名 URL），
            # 未注入/未启用/无 username 时回退旧的本地 assets 写盘逻辑。
            if not image_url and b64:
                import time as _time
                img_bytes = base64.b64decode(b64)
                img_filename = f"generated_{int(_time.time())}.png"

                webdav = self._webdav
                if webdav is not None and username and getattr(webdav, "enabled", False):
                    # WebDAV 模式：上传到用户租户目录，返回签名 URL
                    # （/uploads/{username}/{filename}?sig=...），
                    # 不再写共享 assets 目录
                    image_url = await webdav.put_file(username, img_filename, img_bytes)
                    logger.info(
                        "[IMG] AI生成图片已上传WebDAV: user=%s file=%s URL=%s",
                        username, img_filename, image_url,
                    )
                else:
                    # 本地回退模式：保存到 Flet assets_dir 根目录
                    import os as _os
                    # __file__ 是 src/services/agent_orchestrator.py，上溯 3 级到项目根目录
                    assets_dir = _os.path.join(
                        _os.path.dirname(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))),
                        "assets"
                    )
                    _os.makedirs(assets_dir, exist_ok=True)
                    img_filepath = _os.path.join(assets_dir, img_filename)
                    with open(img_filepath, "wb") as f:
                        f.write(img_bytes)
                    image_url = f"/uploads/{img_filename}"
                    logger.info("[IMG] AI生成图片已保存: %s, URL: %s", img_filepath, image_url)
