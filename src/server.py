...
async def upload_image(request: Request, file: UploadFile = File(...)):
    """上传图片，自动压缩到1024px以内；启用 WebDAV 时写入用户目录，否则落本地 assets。"""
    ...
    filename = f"upload_{int(time.time())}_{uuid.uuid4().hex[:6]}.jpg"
    if webdav.enabled:
        url = await webdav.put_file(username, filename, content)
...
async def ws_chat(ws: WebSocket):
    ...
    try:
        while True:
            ...
            # 处理图片（兼容新 /uploads/{username}/{filename} 与旧本地格式）
            image_data = ""
...
async def panorama_stitch(request: Request, files: list[UploadFile] = File(...)):
    ...
    filename = f"panorama_{int(time.time())}_{uuid.uuid4().hex[:6]}.png"
    if webdav.enabled:
        buf = io.BytesIO()
        equi_img.save(buf, 'PNG')
        url = await webdav.put_file(username, filename, buf.getvalue())
    else:
        filepath = _ASSETS_DIR / filename
        equi_img.save(filepath, 'PNG')
        url = f"/uploads/{filename}"

    await _save_panorama_history(user_id, {"mode": "stitch"}, url)
    return {"url": url, "filename": filename}
...
async def panorama_ai_generate(
    ...
    # 结果图落盘：WebDAV 用户目录 或 本地 assets
    if webdav.enabled:
        url = await webdav.put_file(username, pano_filename, pano_bytes)
    else:
        (_ASSETS_DIR / pano_filename).write_bytes(pano_bytes)
        url = f"/uploads/{pano_filename}"

    # 历史写入数据库（替代原 .meta.json 文件方案）
    await _save_panorama_history(
        user_id, {"mode": "ai-direct", "style": style_desc}, url
    )

    return {"url": url, "filename": pano_filename}
...
async def panorama_ai_correct(request: Request, data: dict):
    ...
    try:
        ...
        if result.get("success") and result.get("images"):
            ...
            out_name = f"panorama_corrected_{int(time.time())}_{uuid.uuid4().hex[:6]}.png"
            buf = io.BytesIO()
            corrected.save(buf, "PNG")
            out_bytes = buf.getvalue()
            if webdav.enabled:
                url = await webdav.put_file(username, out_name, out_bytes)
            else:
                out_path = _ASSETS_DIR / out_name
                out_path.write_bytes(out_bytes)
                url = f"/uploads/{out_name}"
            # 历史写入数据库
            await _save_panorama_history(user_id, {"mode": "ai-correct"}, url)
            return {"url": url, "filename": out_name}
        else:
            # AI 修正失败，返回原图
            return {"url": panorama_url, "corrected": False}
